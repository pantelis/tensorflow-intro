...
flags.DEFINE_integer('max_steps', 2000, 'Maximum number of training steps')
    ...
    with tf.Session(graph=mnist_graph) as sess:
        ...
        for step in range(configuration.max_steps):
            ...
            if step % 1000 == 0:
                ...
                summary_str = sess.run(summary, feed_dict=train_dict)
        ...
        print('Step {:d}, test accuracy {:g}'.format(step, test_accuracy))
        sess.run(summary, feed_dict=test_dict)

if __name__ == '__main__':
    tf.app.run(main=main) # , argv=[sys.argv[0]] + unparsed
    sess = tf.Session()
    new_saver = tf.train.import_meta_graph('tf_logs/20170331-210319/checkpoint-19999.meta')
    new_saver.restore(sess, tf.train.latest_checkpoint('tf_logs/20170331-210319'))
